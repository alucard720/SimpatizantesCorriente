...
try {
  if (env.NODE_ENV !== "development") throw new Error("Este comando está permitido solo en desarrollo.");
  ...
  await db.$transaction(async tx => {
    ...
    if (!role.active) throw new Error("El rol ADMIN está inactivo.");
    const previous = await tx.user.findUnique({ where: { email } });
    if (previous && previous.roleId !== role.id) throw new Error("El correo pertenece a otro rol.");
  ...
  if (!await verifyPassword(password, user.passwordHash)) throw new Error("Falló la verificación de contraseña.");
