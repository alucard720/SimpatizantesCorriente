...
const key = z
  ...
  .regex(
    /^[A-Za-z0-9+/]{43}=$/,
    "Debe ser una clave aleatoria de 32 bytes en base64",
  );
const parsed = z
  .object({
    NODE_ENV: z
...
if (!parsed.success) {
  ...
  throw new Error(
    `Configuración incompleta o inválida: ${fields.join(", ")}. Revise backend/.env usando backend/.env.example como referencia.`,
  );
}
...
if (env.NODE_ENV === "production") {
  ...
  if (
    [env.PRIVACY_CONTROLLER, env.PRIVACY_CONTACT, env.PRIVACY_RETENTION].some(
      (v) => v.startsWith("Configurar"),
    )
  )
