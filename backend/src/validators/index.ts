...
export const registrationInput = z
  ...
  .strict()
  .refine(
    (v) => !(v.schoolId && v.schoolName),
    "Seleccione una escuela o escriba el nombre, no ambos",
  );
...
export const userInput = z
  ...
  .strict()
  .refine(
    (v) => (v.role === "LEADER" ? !!v.leaderId : !v.leaderId),
    "LEADER requiere leaderId; ADMIN no debe tener líder",
  );
