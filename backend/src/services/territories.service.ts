...
export async function fetchTerritories(fetcher: typeof fetch = fetch) {
  async function get(path: string) {
    const response = await fetcher(`${baseUrl}/${path}`, {
      headers: { Accept: "application/json" },
      signal: AbortSignal.timeout(20_000),
    });
    if (!response.ok) throw new Error(`API territorial: HTTP ${response.status}`);
    return response.json();
  }
  const provinces = z.object({ valid: z.literal(true), data: z.array(provinceSchema).length(32) })
    .parse(await get("provinces")).data;
  const municipalities: z.infer<typeof municipalitySchema>[] = [];
...
export async function syncTerritories(db: PrismaClient, catalog: Awaited<ReturnType<typeof fetchTerritories>>) {
  return db.$transaction(async tx => {
    ...
    for (const p of catalog.provinces) {
      ...
      if (matches.length > 1) throw new Error(`Conflicto de provincia: ${p.name}`);
    ...
    for (const m of catalog.municipalities) {
      ...
      if (existing) await tx.municipality.update({ where: { id: existing.id }, data });
      else await tx.municipality.create({ data });
    }
