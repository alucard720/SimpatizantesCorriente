...
export async function assignMunicipalities(
  idRaw: unknown,
  raw: unknown,
  ctx: AuditContext,
) {
...
export async function auditLogs(raw: unknown, ctx: AuditContext) {
  const q = pageInput.parse(raw);
  return db.$transaction(async (tx) => {
...
export async function createLeader(raw: unknown, ctx: AuditContext) {
  const data = z
  ...
  return db.$transaction(async (tx) => {
    ...
    await audit(
      ...
      "Leader",
      leader.id,
      "Perfil de líder creado",
    );
    return leader;
  });
}
export function listLeaders(raw: unknown) {
  const q = pageInput.parse(raw);
