...
export async function register(raw: unknown, ctx: AuditContext) {
  ...
  return {
    message:
      "Solicitud recibida. Si los datos permiten un registro nuevo, será procesado.",
  };
}
...
export async function sensitiveData(
  idRaw: unknown,
  raw: unknown,
  ctx: AuditContext,
) {
...
export async function updateStatus(
  ...
  return db.$transaction(async (tx) => {
    ...
    if (!result.count) throw new HttpError(404, "Registro no encontrado");
    await audit(
      tx,
      ctx,
